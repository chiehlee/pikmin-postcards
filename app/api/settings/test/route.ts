...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => ({})) as { api_key?: string; model?: string };
    return jsonResponse(await testSettingsConnection({ apiKey: body.api_key, model: body.model }, {
