...
test("postcard and long-form research dialogs retain independent scroll containers", async () => {
  ...
  const researchImagesPosition = page.indexOf('<section className="research-image-gallery"');
  const managementPosition = page.indexOf('<section className="postcard-management"');
  assert.ok(researchNotePosition < mapPosition, "Research Note must stay above the map");
  assert.ok(mapPosition < researchImagesPosition, "Research images must stay below the map");
  assert.ok(researchImagesPosition < managementPosition, "Management controls must stay below research images");
});
