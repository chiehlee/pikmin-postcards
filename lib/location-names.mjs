...
export function researchedLocationDisplay(location) {
  ...
  const localLabel = countryCode && !homeCountryCodes.has(countryCode) && countryEndonym
    ? appendCountry(endonym, countryEndonym, location?.language)
    : endonym;
  if (!locationNeedsZhTw(location?.language) || !zhTw || zhTw === endonym) return localLabel;
  const translatedLabel = countryCode && !homeCountryCodes.has(countryCode) && countryZhTw
    ? appendCountry(zhTw, countryZhTw, "zh-Hant-TW")
    : zhTw;
...
export function researchedLocationQuery(location) {
  ...
  return address && countryCode && !homeCountryCodes.has(countryCode) && countryEndonym
    ? appendCountry(address, countryEndonym, location?.language)
    : address;
}
...
function appendCountry(label, country, language) {
  if (label === country || label.endsWith(`, ${country}`) || label.endsWith(`，${country}`)) return label;
  const normalizedLanguage = clean(language)?.toLowerCase();
  const separator = normalizedLanguage?.startsWith("zh") || normalizedLanguage?.startsWith("ja")
    ? "，"
    : ", ";
  return `${label}${separator}${country}`;
}
